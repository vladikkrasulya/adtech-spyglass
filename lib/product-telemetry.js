 ...
 *   diff_use         first comparison run in the Diff tab in a session
 *   migrate_use      first time migration advice was displayed in a session
 *   migrate_apply    the user actually applied proposed operations
 *   register         a new account was created
 *   verify_email     an email verification link was confirmed
 *
 * `migrate_use` and `migrate_apply` are deliberately separate. Advice being
 * shown says nothing about whether it was trusted; the ratio between the two is
 * the only honest read on whether the advisor is worth its rule table.
 *
 * ── One writer ─────────────────────────────────────────────────────────────
