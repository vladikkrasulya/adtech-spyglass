...
let _ownerIpsRaw = null;
/** @type {Array<{v4: number, prefix: number} | {literal: string}>} */
let _ownerIpsParsed = [];

/**
 * Parse ORTBTOOLS_OWNER_IPS on demand, memoised on the raw string so a test (or
 * an env reload) that changes it takes effect without a process restart.
 */
function ownerIpRules() {
  const raw = process.env.ORTBTOOLS_OWNER_IPS || '';
  if (raw === _ownerIpsRaw) return _ownerIpsParsed;
  _ownerIpsRaw = raw;
  _ownerIpsParsed = [];
  for (const entry of raw.split(',')) {
    ...
    if (slash !== -1) {
      ...
      // IPv6 CIDR is deliberately unsupported — list IPv6 hosts literally.
      if (net !== null && Number.isInteger(prefix) && prefix >= 0 && prefix <= 32) {
        _ownerIpsParsed.push({ v4: net, prefix });
      }
      continue;
    }
    const v4 = parseIpv4(token);
    if (v4 !== null) _ownerIpsParsed.push({ v4, prefix: 32 });
    else _ownerIpsParsed.push({ literal: token });
  }
  return _ownerIpsParsed;
}
