 ...
 * resolved client IP (lib/client-ip.js applies the trusted-proxy rule) plus the
...
/**
 * Canonicalise an address for comparison: strip brackets and an IPv6 zone
 * index, and reduce every valid IPv4-mapped IPv6 spelling to dotted IPv4.
 * WHATWG URL canonicalises `::ffff:127.0.0.1` as `::ffff:7f00:1`; handling
 * only the dotted spelling would let the same address change class merely by
 * passing through `new URL()`. Returns '' when nothing usable is left.
 * @param {unknown} value
 * @returns {string}
 */
...
/**
 * Return dotted IPv4 for an RFC 4291 IPv4-mapped IPv6 literal, or ''. Both
 * compressed and fully expanded spellings are accepted; other IPv6 forms are
 * deliberately left alone.
 *
 * @param {string} ip lower-case, unbracketed address
 * @returns {string}
 */
function ipv4FromMappedIpv6(ip) {
  let source = ip;
  const lastColon = source.lastIndexOf(':');
  const dotted = lastColon === -1 ? null : parseIpv4(source.slice(lastColon + 1));
  if (dotted !== null) {
    source =
      source.slice(0, lastColon + 1) +
      ((dotted >>> 16) & 0xffff).toString(16) +
      ':' +
      (dotted & 0xffff).toString(16);
  }
  if (!source.includes(':') || !/^[0-9a-f:]+$/.test(source)) return '';
