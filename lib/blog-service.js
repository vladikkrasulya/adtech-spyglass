...
// One shallow frontmatter grammar for the SSR and public API readers. New
// promotions encode scalar strings as JSON so quotes, backslashes and line
// breaks roundtrip without becoming additional metadata fields. An explicit
// encoding marker distinguishes new files from legacy quoted text, where
// backslashes were literal. Keep the existing bare/quoted/array grammar there.
function parseFrontmatter(content) {
  const match = String(content).match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/);
  if (!match) return { meta: {}, body: String(content) };
  const meta = {};
  const lines = match[1].split(/\r?\n/);
  const jsonScalars = lines.includes('frontmatter_encoding: json-v1');
  for (const line of lines) {
    const kv = line.match(/^(\w+):\s*(.*)$/);
    if (!kv) continue;
    let val = kv[2].trim();
    if (jsonScalars && val.startsWith('"') && val.endsWith('"')) {
      try {
        meta[kv[1]] = JSON.parse(val);
        continue;
      } catch {
        // Keep malformed hand-edited values readable using the legacy grammar.
      }
    }
