...
// ── Auth-event privacy boundary (v1.2.1, CP1A.1) ────────────────────────────
// Auth telemetry must never carry PII. `ctx` for component='auth' events may
// contain ONLY these coarse, finite, non-identifying keys — every other key is
// dropped at the boundary (defense in depth, in case a call site regresses).
// Values are validated against finite enums/types, so an email-shaped string
// smuggled under an allowed key is also rejected. Omission is preferred over
// hashing: a stable email hash is still a persistent identifier.
const AUTH_REASON_CODES = new Set(['ok', 'rate_limited', 'wrong_password', 'no_such_user']);
const AUTH_ACTIONS = new Set(['login', 'register', 'logout', 'verify', 'reset', 'recover']);
const AUTH_CTX_ALLOW = {
  outcome: (v) => v === 'success' || v === 'failure',
  reason_code: (v) => AUTH_REASON_CODES.has(v),
  auth_action: (v) => AUTH_ACTIONS.has(v),
  locale: (v) => v === 'en' || v === 'uk' || v === 'ru',
  status_code: (v) => Number.isInteger(v) && v >= 0 && v < 600,
  rate_limited: (v) => v === true || v === false,
};
// Build a fresh sanitized ctx for an auth event: copy ONLY allowlisted keys
// whose value passes the field validator; drop everything else. Fail-closed,
// deterministic, never mutates the caller's object, never reads/logs a rejected
// value. Returns {} when nothing survives.
function sanitizeAuthCtx(ctx) {
  const out = {};
  if (ctx && typeof ctx === 'object' && !Array.isArray(ctx)) {
    for (const key of Object.keys(AUTH_CTX_ALLOW)) {
      if (Object.prototype.hasOwnProperty.call(ctx, key) && AUTH_CTX_ALLOW[key](ctx[key])) {
        out[key] = ctx[key];
      }
    }
  }
...
function record(entry) {
  try {
    ...
    // Auth-event privacy boundary: auth telemetry carries ONLY a finite,
    // non-identifying ctx allowlist and NO ip / user_id / method / path. This is
    // enforced here (not just at the call site) so a future regressed producer
    // still cannot leak email/PII. Every other component is unchanged.
    const isAuth = component === 'auth';
    const ctxObj = isAuth ? sanitizeAuthCtx(entry.ctx) : entry.ctx;
    const ctx = ctxObj && typeof ctxObj === 'object' ? safeStringify(ctxObj).slice(0, 2000) : '';
    ...
    _buffer.push({
      ...
      component,
      msg,
      method: isAuth ? '' : entry.method ? String(entry.method).slice(0, 8) : '',
      path: isAuth ? '' : entry.path ? String(entry.path).slice(0, 500) : '',
      status: Number.isFinite(entry.status) ? entry.status : 0,
      latency_ms: Number.isFinite(entry.latency_ms) ? entry.latency_ms : 0,
      user_id: isAuth ? 0 : Number.isFinite(entry.user_id) ? entry.user_id : 0,
      ip: isAuth ? '' : entry.ip ? String(entry.ip).slice(0, 64) : '',
...
module.exports = {
  ...
  flushNow,
  RETENTION_DAYS,
  // exposed for the auth-event privacy boundary contract tests (CP1A.1)
  sanitizeAuthCtx,
};
