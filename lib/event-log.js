 ...
 * subset to ClickHouse (`analytics.ortbtools_events`) so the operator can
...
// "Enabled" gate: if no CH user is configured or ORTBTOOLS_ANALYTICS_DISABLED=1
// we treat the module as a no-op so ortbtools still boots in tests/dev without
// a CH dependency. We log the degradation once and stay silent afterward.
const CH_ENABLED = isClickHouseAnalyticsEnabled();
...
async function postBatch(rows) {
  ...
  try {
    ...
    const url =
      CH_URL +
      '/?query=' +
      encodeURIComponent('INSERT INTO analytics.ortbtools_events FORMAT JSONEachRow');
...
async function query(filters) {
  ...
  const totalSql = `SELECT count() AS n FROM analytics.ortbtools_events ${whereSql} FORMAT JSONEachRow`;
  const rowsSql =
    `SELECT toUnixTimestamp64Milli(ts) AS ts, level, component, msg, method, path, ` +
    `status, latency_ms, user_id, ip, request_id, ctx ` +
    `FROM analytics.ortbtools_events ${whereSql} ` +
 ...
 * `TTL ts + INTERVAL 90 DAY` clause on `analytics.ortbtools_events`.
