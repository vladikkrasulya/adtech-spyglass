...
// /api/v1/sample → samples/synthetic-<slug>.json | <slug>.json). Falsy = no CTA
// button at all — which no landing should choose lightly: a landing with no
// sample is a page that talks about the product without offering a way in.
//
...
const LANDINGS = {
  '/openrtb/2-6': {
    ...
    lede: {
      en: 'Paste an OpenRTB 2.6 BidRequest or BidResponse and get a human-readable validation — no signup. Your payload is analyzed on the server and not persisted there; the browser may retain it in local History. This page explains what 2.6 added over 2.5 and the fields most likely to trip a validator.',
      uk: 'Встав OpenRTB 2.6 BidRequest або BidResponse і отримай зрозумілу валідацію онлайн — без реєстрації. Payload аналізується й не персиститься на сервері; браузер може залишити його в локальній History. Нижче — що саме 2.6 додав до 2.5 і які поля найчастіше валять валідацію.',
      ru: 'Вставь OpenRTB 2.6 BidRequest или BidResponse и получи понятную валидацию онлайн — без регистрации. Payload анализируется и не сохраняется на сервере; браузер может оставить его в локальной History. Ниже — что именно 2.6 добавил к 2.5 и какие поля чаще всего валят валидацию.',
    },
    sections: [
      {
        h2: {
  ...
  '/iab-categories': {
    // The lede has always ended "…then open any bid in the validator to see
    // codes decoded inline", and this was the one landing that offered no way
    // to do that — the only link on the page went to iabtechlab.com, i.e. the
    // single page whose job is to explain IABx codes was also the single page
    // with no door into the product that decodes them. The sample is picked to
    // pay the sentence off literally: iab-banner-valid carries
    // site.cat = ["IAB19-6"], so the reader lands on a bid where a code from
    // the table above is sitting in cat[] and rendered decoded.
    sample: 'iab-banner-valid',
    cta: {
      en: 'Open a bid with site.cat = IAB19-6 in the validator',
      uk: 'Відкрити ставку з site.cat = IAB19-6 у валідаторі',
      ru: 'Открыть ставку с site.cat = IAB19-6 в валидаторе',
    },
    kind: 'iab-table',
    specLinks: [
      {
...
function renderLandingBody(sectionPath, lang) {
  ...
  // Mono eyebrow kicker derived from the URL — the shared section grammar
  // (.eyebrow from design-system.css) that the portal heroes and /about use.
  // '/openrtb/2-6' → "openrtb · 2.6", '/iab-categories' → "iab categories".
  const eyebrow = escapeHtml(
    sectionPath
      .replace(/^\//, '')
      .split('/')
      .map((seg) => (/^\d+-\d+$/.test(seg) ? seg.replace('-', '.') : seg.replace(/-/g, ' ')))
      .join(' · '),
  );
  ...
  if (cfg.sample) {
    ...
    cta = `<p class="landing__cta"><a class="btn btn-primary" href="${escapeHtml(href)}">${escapeHtml(
      pick(cfg.cta, L),
    )} →</a></p>`;
  }
  ...
  // Signal motion-pack marquee (design-system .ticker) with real engine
  // facts. The track content is doubled — the keyframes translate to -50%,
  // so a single copy would leave a gap at the loop point.
  const tickerItems = [
    'openrtb 2.5 / 2.6 / 3.0',
    'vast 2 · 3 · 4',
    'native 1.1 / 1.2',
    'jsonfeed',
    pick(
      { en: '300+ validation rules', uk: '300+ правил валідації', ru: '300+ правил валидации' },
      L,
    ),
    pick(
      {
        en: 'request ↔ response crosscheck',
        uk: 'звірка запит ↔ відповідь',
        ru: 'сверка запрос ↔ ответ',
      },
      L,
    ),
    pick(
      {
        en: 'no signup · no upload',
        uk: 'без реєстрації · без завантажень',
        ru: 'без регистрации · без загрузок',
      },
      L,
    ),
  ]
  ...
  const ticker =
    `<div class="ticker landing__ticker" aria-hidden="true"><div class="ticker-track">` +
    tickerItems +
  ...
  return (
    `<section class="landing"><header class="landing__hero kt-blueprint">` +
...
// The filter box is the one control on a landing that does nothing without
// JavaScript: the SSR table is complete and readable, but the `input` handler
// lives in public/modules/landing/index.js, so a no-JS reader gets a search
// field that swallows every keystroke and never filters a row. Hiding it is the
// honest answer — the affordance is a lie for exactly the readers who cannot
// use it, and the 26-row table it sits above needs no filter to be read.
//
// This cannot live in landing.css: "is scripting on?" is not expressible in a
// stylesheet. <noscript> is, and inside <head> a <style> child is the spec's
// own conforming shape for it (HTML: "when scripting is disabled, in a head
// element: zero or more link, style and meta elements"). With scripting on the
// element is inert, so the JS path is untouched and there is no flash.
const NOSCRIPT_FILTER_CSS = '<noscript><style>.landing__filter{display:none}</style></noscript>';
...
function injectLanding(html, sectionPath, lang) {
  ...
  // Only the table landings render a filter; guarded like the css link so a
  // double injection cannot stack two copies.
  if (cfg.kind === 'iab-table' && !html.includes(NOSCRIPT_FILTER_CSS)) {
    html = html.replace(/<\/head>/i, `    ${NOSCRIPT_FILTER_CSS}\n  </head>`);
  }
  return html;
}
