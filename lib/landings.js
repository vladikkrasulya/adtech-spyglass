...
const LANDINGS = {
  '/openrtb/2-6': {
    ...
    lede: {
      en: 'Paste an OpenRTB 2.6 BidRequest or BidResponse and get a human-readable validation in the browser — no upload, no signup. This page explains what 2.6 added over 2.5 and the fields most likely to trip a validator.',
      uk: 'Встав OpenRTB 2.6 BidRequest або BidResponse і отримай зрозумілу валідацію прямо в браузері — без завантаження й реєстрації. Нижче — що саме 2.6 додав до 2.5 і які поля найчастіше валять валідацію.',
      ru: 'Вставь OpenRTB 2.6 BidRequest или BidResponse и получи понятную валидацию прямо в браузере — без загрузки и регистрации. Ниже — что именно 2.6 добавил к 2.5 и какие поля чаще всего валят валидацию.',
    },
    sections: [
      {
        h2: {
  ...
  '/openrtb/2-5': {
    ...
    lede: {
      en: 'OpenRTB 2.5 is the workhorse of programmatic — most live bid traffic still speaks it. Paste a BidRequest or BidResponse and validate it in the browser, no upload or signup.',
      uk: 'OpenRTB 2.5 — робоча конячка programmatic: більшість живого трафіку досі говорить саме ним. Встав BidRequest або BidResponse і перевір його в браузері, без завантаження й реєстрації.',
      ru: 'OpenRTB 2.5 — рабочая лошадка programmatic: большая часть живого трафика до сих пор говорит именно им. Вставь BidRequest или BidResponse и проверь его в браузере, без загрузки и регистрации.',
    },
    sections: [
      {
  ...
  '/openrtb/3-0': {
    ...
    lede: {
      en: 'OpenRTB 3.0 re-architects the model: a thin transport envelope plus AdCOM for the ad and placement objects. Paste a 3.0 object and validate the new shape in the browser.',
      uk: 'OpenRTB 3.0 переархітектурує модель: тонкий транспортний конверт плюс AdCOM для об’єктів реклами й розміщення. Встав 3.0-об’єкт і перевір нову структуру в браузері.',
      ru: 'OpenRTB 3.0 переархитектурирует модель: тонкий транспортный конверт плюс AdCOM для объектов рекламы и размещения. Вставь 3.0-объект и проверь новую структуру в браузере.',
    },
    sections: [
      {
  ...
  '/vast': {
    ...
    lede: {
      en: 'VAST is the XML that delivers a video or CTV creative — usually returned inside bid.adm. Open a clean inline VAST plus its bid envelope and inspect the structure in the browser.',
      uk: 'VAST — це XML, що доставляє відео- чи CTV-креатив, зазвичай у bid.adm. Відкрий чистий inline VAST разом із його bid-конвертом і досліди структуру в браузері.',
      ru: 'VAST — это XML, доставляющий видео- или CTV-креатив, обычно в bid.adm. Открой чистый inline VAST вместе с его bid-конвертом и изучи структуру в браузере.',
    },
    sections: [
      {
  ...
  '/native': {
    ...
    lede: {
      en: 'Native ads carry a request-inside-the-request: a stringified Native markup object describing the assets the publisher will render. Open a valid native request and crosscheck assets request↔response in the browser.',
      uk: 'Нативна реклама несе запит усередині запиту: рядок-об’єкт Native-розмітки, що описує ассети, які відрендерить видавець. Відкрий валідний native-запит і звір ассети запит↔відповідь у браузері.',
      ru: 'Нативная реклама несёт запрос внутри запроса: строку-объект Native-разметки, описывающую ассеты, которые отрендерит издатель. Открой валидный native-запрос и сверь ассеты запрос↔ответ в браузере.',
    },
    sections: [
      {
        h2: {
