 ...
 *   - cluster/purpose calls inspect only field paths and metadata; partner and
 *     simulation calls may receive raw payloads but evaluate a bounded set of
 *     domain, bundle, format, floor, and completeness signals. This pure module
...
// Locale-aware strategy `reason` text. Ad-tech jargon (floor, ROAS, ROI,
// the strategy/format/surface tokens themselves) stays untranslated in
// uk/ru — same convention as public/modules/simulate/i18n.js, which keeps
// "floor у {cur}" and "quality · premium only" verbatim across locales.
// Callers pass only validated locales; an unknown key still falls back to
// `en` here so this stays a total, deterministic function.
const SIM_REASON_TEXT = {
  en: {
    aggressiveCapped: (floor, cur) => `floor ${floor} ${cur} beyond scale budget (cap 25)`,
    aggressiveBid: (fmt, q) =>
      `scale play on ${fmt}: floor×1.40, quality ${q} acceptable at volume`,
    conservativeBid: (q, floor, cur) =>
      `shape fits (quality ${q}, floor ${floor} ${cur}): floor×1.10 protects ROAS`,
    conservativeThin: (q) => `metadata too thin (quality ${q} < 0.5) for a safe ROI call`,
    conservativeCeiling: (floor, cur) => `floor ${floor} ${cur} above conservative ceiling (10)`,
    qualityBid: (q, surface, bundle) =>
      `premium signals (quality ${q}, ${surface}${bundle}): floor×1.65`,
    qualityMiss: (q) =>
      `premium bar not met (quality ${q} < 0.65): incomplete metadata or plain shape`,
  },
  uk: {
    aggressiveCapped: (floor, cur) =>
      `floor ${floor} ${cur} за межами бюджету масштабування (стеля 25)`,
    aggressiveBid: (fmt, q) =>
      `ставка на масштаб для ${fmt}: floor×1.40, якість ${q} прийнятна для обсягу`,
    conservativeBid: (q, floor, cur) =>
      `форма підходить (якість ${q}, floor ${floor} ${cur}): floor×1.10 захищає ROAS`,
    conservativeThin: (q) => `метаданих замало (якість ${q} < 0.5) для безпечного розрахунку ROI`,
    conservativeCeiling: (floor, cur) => `floor ${floor} ${cur} вище консервативної стелі (10)`,
    qualityBid: (q, surface, bundle) =>
      `преміум-сигнали (якість ${q}, ${surface}${bundle}): floor×1.65`,
    qualityMiss: (q) =>
      `преміум-поріг не досягнуто (якість ${q} < 0.65): неповні метадані або проста форма`,
  },
  ru: {
    aggressiveCapped: (floor, cur) =>
      `floor ${floor} ${cur} за пределами бюджета масштабирования (потолок 25)`,
    aggressiveBid: (fmt, q) =>
      `ставка на масштаб для ${fmt}: floor×1.40, качество ${q} приемлемо для объёма`,
    conservativeBid: (q, floor, cur) =>
      `форма подходит (качество ${q}, floor ${floor} ${cur}): floor×1.10 защищает ROAS`,
    conservativeThin: (q) => `метаданных мало (качество ${q} < 0.5) для безопасного расчёта ROI`,
    conservativeCeiling: (floor, cur) => `floor ${floor} ${cur} выше консервативного потолка (10)`,
    qualityBid: (q, surface, bundle) =>
      `премиум-сигналы (качество ${q}, ${surface}${bundle}): floor×1.65`,
    qualityMiss: (q) =>
      `премиум-порог не достигнут (качество ${q} < 0.65): неполные метаданные или простая форма`,
  },
};
 ...
 * @param {object} bidReq
 * @param {string} [locale='en'] validated 'en'|'uk'|'ru'; anything else falls back to 'en'.
 * @returns {Array<{strategy, label, bid, price, reason, engine}> | null}
 */
function simulateBids(bidReq, locale) {
  if (!bidReq || typeof bidReq !== 'object') return null;
  const L = SIM_REASON_TEXT[locale] || SIM_REASON_TEXT.en;
  ...
  if (floor > 25) {
    out.push({
      ...
      bid: false,
      price: null,
      reason: L.aggressiveCapped(floor, cur),
      engine: 'rules',
    });
  } else {
    out.push({
  ...
  if (q >= 0.5 && floor <= 10) {
    out.push({
      ...
      reason: L.conservativeBid(q, floor, cur),
      engine: 'rules',
    });
  } else {
    out.push({
      ...
      bid: false,
      price: null,
      reason: q < 0.5 ? L.conservativeThin(q) : L.conservativeCeiling(floor, cur),
      engine: 'rules',
    });
  }
  ...
  if (q >= 0.65) {
    out.push({
      ...
      reason: L.qualityBid(
        q,
        s.surface,
        s.appBundleOrDomain ? ' ' + String(s.appBundleOrDomain).slice(0, 30) : '',
      ),
      engine: 'rules',
    });
  } else {
