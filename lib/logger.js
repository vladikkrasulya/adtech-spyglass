...
/**
 * lib/logger.js — pino-based structured logger for the ortbtools server.
 *
...
// `captureException` calls for the configured Sentry-compatible destination.
// When unset, the helpers no-op cleanly so development and test boots do not
// depend on an error-reporting service.
...
if (process.env.SENTRY_DSN && !isTest) {
  try {
    ...
    // `init()` is intentionally non-throwing for some invalid DSNs. Only
    // advertise a configured SDK when it retained a parsed DSN. This does NOT
    // probe the upstream or promise that event delivery is currently healthy.
    const client = typeof Sentry.getClient === 'function' ? Sentry.getClient() : null;
    _sentryReady = !!(client && typeof client.getDsn === 'function' && client.getDsn());
  } catch (_e) {
    // Unexpected SDK/config error — never let observability crash boot.
    _sentryReady = false;
  }
}

/**
 * Report an exception to the configured Sentry-compatible destination. No-ops
 * when SENTRY_DSN isn't configured (dev/test) so callers don't need to
 * feature-check. Optional
...
/** Does Sentry have a valid SDK configuration? This is not an upstream probe. */
