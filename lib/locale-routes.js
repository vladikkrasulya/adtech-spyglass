...
/** Read a supported UI locale without letting malformed cookies break a request. */
function readLocaleCookie(req) {
  const cookie = req.headers.cookie || '';
  for (const part of cookie.split(';')) {
    const separator = part.indexOf('=');
    if (separator < 0 || part.slice(0, separator).trim() !== 'kt-lang') continue;
    let locale;
    try {
      locale = decodeURIComponent(part.slice(separator + 1)).trim();
    } catch {
      return null;
    }
    if (locale === 'en' || locale === 'uk' || locale === 'ru') return locale;
  }
  return null;
}
