...
/** Decode percent-encoding (bounded loop for double-encoding); null if unsafe after decode. */
...
function resolveLocaleRoute(reqUrl) {
  ...
  const pathRedirect = blogRedirect || (normalized !== decoded ? normalized : null);
  if (pathRedirect) {
