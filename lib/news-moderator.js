 ...
 *   3. Score relevance with the deterministic rules engine (no model).
 *      score < threshold → reject.
 *   4. score >= threshold → translate + categorize via OpenRouter DeepSeek —
 *      the ISOLATED content pipeline (never the local brain).
...
// 2026-07-22: relevance scoring is deterministic (lib/intel-rules.js) — the
// only model left in this file is the ISOLATED OpenRouter translation step
// below, a content pipeline that never touches the local 26B brain.
const { scoreRelevance } = require('./intel-rules');
...
async function moderatePendingDrafts() {
  ...
  for (const draft of drafts) {
    ...
    // 1) relevance score — deterministic keyword/weight table, no model.
    // Same input → same score; a scorer that cannot fail cannot strand
    // drafts in pending the way an unreachable Ollama used to.
    const { score, hits } = scoreRelevance(draft);
    log.info({ id: draft.id, score, hits }, 'relevance scored (rules)');
