...
(function () {
  ...
  const I18N = {
    uk: {
      ...
      'toast.nothing_to_mirror': 'Нічого віддзеркалювати — встав BidRequest або BidResponse',
      'toast.mirror_invalid_json': 'Не вдалось розпарсити JSON у вхідному полі',
      'toast.mirror_copied': 'Згенерований JSON скопійовано в буфер',
      'toast.mirror_copy_failed': 'Не вдалось скопіювати в буфер',
      'toast.mirror_loaded': 'Згенерований JSON завантажено у відповідне поле',
      ...
      'modal.mirror.title': 'дзеркало запит ↔ відповідь',
      'modal.mirror.unsupported_title': 'дзеркало · не підтримується',
      'modal.mirror.loading': 'генеруємо канонічну пару…',
      'modal.mirror.failed': 'не вдалось згенерувати',
      'modal.mirror.dir.response_from_request': 'BidRequest → BidResponse',
      'modal.mirror.dir.request_from_response': 'BidResponse → BidRequest',
      'modal.mirror.output_label': 'згенерований JSON',
      'modal.mirror.notes_label': 'що вирішено й чому',
      'modal.mirror.btn_copy': 'копіювати',
      'modal.mirror.btn_load': 'завантажити в інше поле',
      'modal.mirror.btn_share': '🔗 share-лінк з парою',
      'toast.mirror_share_copied': 'Permalink з канонічною парою скопійовано в буфер',
      'toast.mirror_share_failed': 'Не вдалось зробити share-лінк: {error}',
      ...
      'finding.severity.warning.text': 'Більшість бірж толерують, але fill-rate знизиться — варто виправити.',
      'finding.severity.info.label': 'info',
      'finding.severity.info.text': 'Best-practice примітка — не баг, але краще робити так.',
      'corpus.bar.label': '{count} події у поточному probe — можна зберегти як приклад для майбутнього confusion matrix.',
      'corpus.bar.save_btn': 'зберегти як corpus',
      'corpus.cabinet.total': 'усього',
      'corpus.cabinet.empty': 'Поки нічого не збережено. Згенеруй або завантаж зразок, відкрий behavior tab — і там зʼявиться кнопка «зберегти як corpus».',
      ...
      'modal.corpus_save.summary': 'Поточний probe має {count} події. Збережемо їх з міткою для подальшого аналізу.',
      ...
      'modal.corpus_save.notes_placeholder': 'Що тригернуло цю мітку, що варто запамʼятати на майбутнє…',
      ...
      'modal.simbids.hint': 'Локальна gemma3:4b симулює як три типи DSP відповіли б на твій запит. Тільки метадані запиту (формат, розмір, гео, floor) — bid VALUES не передаємо в LLM.',
      'modal.simbids.loading': 'gemma думає… (3 паралельні prompts, ~10s)',
      'modal.simbids.ollama_down': 'Ollama недоступна. AI-bridge падає тихо — попроси адміна перевірити контейнер.',
    ...
    en: {
      ...
      'toast.nothing_to_mirror': 'Nothing to mirror — paste a BidRequest or BidResponse first',
      'toast.mirror_invalid_json': 'Could not parse JSON in the source field',
      'toast.mirror_copied': 'Generated JSON copied to clipboard',
      'toast.mirror_copy_failed': 'Could not copy to clipboard',
      'toast.mirror_loaded': 'Generated JSON loaded into the other editor',
      ...
      'modal.mirror.title': 'mirror request ↔ response',
      'modal.mirror.unsupported_title': 'mirror · not supported',
      'modal.mirror.loading': 'building canonical pair…',
      'modal.mirror.failed': 'could not generate',
      'modal.mirror.dir.response_from_request': 'BidRequest → BidResponse',
      'modal.mirror.dir.request_from_response': 'BidResponse → BidRequest',
      'modal.mirror.output_label': 'generated JSON',
      'modal.mirror.notes_label': 'choices and reasons',
      'modal.mirror.btn_copy': 'copy',
      'modal.mirror.btn_load': 'load into the other editor',
      'modal.mirror.btn_share': '🔗 share link with the pair',
      'toast.mirror_share_copied': 'Permalink with the canonical pair copied to clipboard',
      'toast.mirror_share_failed': 'Could not build share link: {error}',
      ...
      'modal.live.hint': 'Click a row to load that sample into the matching editor and close the modal.',
      ...
      'finding.detail.value_missing': 'Field is absent in the pasted JSON (which is exactly why this finding fires).',
      'finding.detail.severity': 'Severity',
      'finding.detail.spec': 'Spec reference',
      'finding.detail.rule_id': 'Rule id',
      'finding.severity.error.label': 'error',
      'finding.severity.error.text': 'Exchanges will reject the request/bid — it won’t reach the auction.',
      'finding.severity.warning.label': 'warning',
      'finding.severity.warning.text': 'Most exchanges tolerate this but fill rate suffers — worth fixing.',
      'finding.severity.info.label': 'info',
      'finding.severity.info.text': 'Best-practice note — not a bug, but recommended.',
      'corpus.bar.label': '{count} events in this probe — save them as a labelled example for the upcoming confusion-matrix runner.',
      'corpus.bar.save_btn': 'save as corpus',
      'corpus.cabinet.total': 'total',
      'corpus.cabinet.empty': 'Nothing saved yet. Generate or load a sample, open the behavior tab — the "save as corpus" button shows up there.',
      ...
      'modal.corpus_save.summary': 'Current probe has {count} events. Save them with a label for later analysis.',
      ...
      'modal.corpus_save.notes_placeholder': 'What triggered this label, anything worth remembering for later…',
      ...
      'modal.simbids.hint': 'Local gemma3:4b simulates how three DSP types would respond to your request. Metadata only (format, size, geo, floor) — bid VALUES never reach the LLM.',
      'modal.simbids.loading': 'gemma is thinking… (3 parallel prompts, ~10s)',
      'modal.simbids.ollama_down': 'Ollama unavailable. AI bridge fails quietly — ask the admin to check the container.',
    ...
    ru: {
      ...
      'toast.nothing_to_mirror': 'Нечего зеркалировать — вставь BidRequest или BidResponse',
      'toast.mirror_invalid_json': 'Не удалось распарсить JSON в исходном поле',
      'toast.mirror_copied': 'Сгенерированный JSON скопирован в буфер',
      'toast.mirror_copy_failed': 'Не удалось скопировать в буфер',
      'toast.mirror_loaded': 'Сгенерированный JSON загружен в соседнее поле',
      ...
      'modal.mirror.title': 'зеркало запрос ↔ ответ',
      'modal.mirror.unsupported_title': 'зеркало · не поддерживается',
      'modal.mirror.loading': 'строим каноническую пару…',
      'modal.mirror.failed': 'не удалось сгенерировать',
      'modal.mirror.dir.response_from_request': 'BidRequest → BidResponse',
      'modal.mirror.dir.request_from_response': 'BidResponse → BidRequest',
      'modal.mirror.output_label': 'сгенерированный JSON',
      'modal.mirror.notes_label': 'что и почему решено',
      'modal.mirror.btn_copy': 'копировать',
      'modal.mirror.btn_load': 'загрузить в соседнее поле',
      'modal.mirror.btn_share': '🔗 share-ссылка с парой',
      'toast.mirror_share_copied': 'Permalink с канонической парой скопирован в буфер',
      'toast.mirror_share_failed': 'Не удалось собрать share-ссылку: {error}',
      ...
      'modal.live.hint': 'Клик по строке — загрузить образец в соответствующее поле и закрыть окно.',
      ...
      'finding.detail.value_missing': 'Поле отсутствует во вставленном JSON (именно поэтому находка).',
      ...
      'finding.severity.warning.label': 'warning',
      'finding.severity.warning.text': 'Большинство бирж толерируют, но fill-rate упадёт — стоит исправить.',
      'finding.severity.info.label': 'info',
      'finding.severity.info.text': 'Best-practice заметка — не баг, но лучше делать так.',
      'corpus.bar.label': '{count} событий в текущем probe — можно сохранить как пример для будущего confusion matrix.',
      'corpus.bar.save_btn': 'сохранить как corpus',
      'corpus.cabinet.total': 'всего',
      'corpus.cabinet.empty': 'Пока ничего не сохранено. Сгенерируй или загрузи образец, открой behavior tab — там появится кнопка «сохранить как corpus».',
      ...
      'modal.corpus_save.summary': 'Текущий probe имеет {count} событий. Сохраним их с меткой для дальнейшего анализа.',
      ...
      'modal.corpus_save.notes_placeholder': 'Что триггернуло эту метку, что стоит запомнить на будущее…',
      ...
      'modal.simbids.hint': 'Локальная gemma3:4b симулирует как три типа DSP ответили бы на твой запрос. Только метаданные (формат, размер, гео, floor) — bid VALUES не передаются в LLM.',
      'modal.simbids.loading': 'gemma думает… (3 параллельных prompts, ~10s)',
      'modal.simbids.ollama_down': 'Ollama недоступна. AI-bridge падает тихо — попроси админа проверить контейнер.',
