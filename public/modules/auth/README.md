...
`window.OrtbtoolsSession` (the closure-scoped facade in `ortbtools.app.js`).
...
`ortbtools.app.js` dispatcher and the inline `lazyOpenAuth()` helper. On
...
- `window.doLogin()` — POST /api/auth/login + DEK unlock via
  `OrtbtoolsSession.openFromPassword` (or bootstrap-on-first-login for
  legacy pre-Phase-7 accounts).
- `window.doRegister()` — POST /api/auth/register + bootstrap fresh
  DEK via `OrtbtoolsSession.bootstrap` + show recovery key once.

## Window APIs (consumes)

- `window.OrtbtoolsSession.{api, refreshPartners, refreshSamples,
...
- `window.snapshotPendingHistoryMerge()` — sets the closure-private
  ...
  one-liner exposed by `ortbtools.app.js`; the flag itself stays in
...
Two `data-action` cases stay in ortbtools.app.js's central dispatcher:
