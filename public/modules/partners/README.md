...
(owned by ortbtools.app.js — `_partnerCache` is shared with the
...
ortbtools.app.js dispatcher. On first click: ~3KB across `index.js` +
...
- `window.refreshPartners` — re-pulls `_partnerCache` (owned by
  ortbtools.app.js; shared with partner-suggest banner and save-modal
...
Uses an inline `api()` helper that mirrors ortbtools.app.js's local
helper (same error shape — `status` + `code` on the thrown Error —
so the `partner_not_found` / 401 paths still surface meaningfully).

## Dispatcher cases

Three `data-action` cases stay in ortbtools.app.js's central dispatcher
...
## Why state stays in ortbtools.app.js
...
goal. So the rule: cache lives in ortbtools.app.js, the modal that
mutates it is lazy.
