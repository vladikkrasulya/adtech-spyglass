...
(function () {
  ...
  window.utils = {
    format(id) {
      try {
        ...
        updateCharCount(id);
        updateJsonBadge(id);
      } catch (e) { toast('Невалідний JSON: ' + e.message, 'error'); }
    },
    copy(id) {
      const el = $(id);
      if (!el.value) { toast('Поле пусте — нічого копіювати', 'error'); return; }
  ...
  window.clearInput = function (id) { $(id).value = ''; updateCharCount(id); };
  ...
  function updateJsonBadge(id) {
    ...
    if (!v) { badge.textContent = 'empty'; badge.className = 'json-badge empty'; return; }
    try { JSON.parse(v); badge.textContent = 'valid'; badge.className = 'json-badge valid'; }
    catch { badge.textContent = 'invalid'; badge.className = 'json-badge invalid'; }
  }
  ...
  function findAdm(obj) {
    ...
    if (obj.nurl && typeof obj.nurl === 'string') return '<img src="' + escapeHtml(obj.nurl) + '" style="width:100%"/>';
    for (const k in obj) {
  ...
  function renderNativePreview(native) {
    ...
    let title = null, img = null, icon = null, desc = null, sponsored = null;
    for (const a of native.assets || []) {
      if (a.title && a.title.text)     title = a.title.text;
      if (a.img && a.img.url) {
        const w = Number(a.img.w || 0), h = Number(a.img.h || 0);
  ...
  window.runAnalysis = async function (fromHist) {
    ...
    if (!reqVal) { toast('Встав BidRequest у ліве поле', 'error'); return; }
    ...
    try {
      ...
      const entity = (req.site || req.app || {}).domain
      ...
      $('mPrice').innerText = adm
        ? (bid.price ? '$' + Number(bid.price).toFixed(2) : 'BID')
        : '$0.00';
      ...
      slotGrid.innerHTML = imps.length ? imps.map((i, idx) => {
        const types = getSlotType(i);
        const typeHtml = '<div class="slot-type-row">' + types.map((t) =>
          '<span class="slot-type ' + t + '">' + t + '</span>'
        ).join('') + '</div>';
        let dims = '';
        if (i.banner && i.banner.w && i.banner.h) dims = i.banner.w + '×' + i.banner.h;
        else if (i.video && i.video.w && i.video.h) dims = i.video.w + '×' + i.video.h + ' video';
        return '<div class="slot-card">' +
          typeHtml +
          '<div class="slot-id">' + escapeHtml(i.id || ('imp-' + idx)) + (i.tagid ? ' · ' + escapeHtml(i.tagid) : '') + '</div>' +
          (dims ? '<div class="slot-dims">' + dims + '</div>' : '') +
          '<div class="slot-floor"><span class="slot-floor-label">floor</span><span class="slot-floor-value">$' + (Number(i.bidfloor) || 0).toFixed(3) + '</span></div>' +
          '</div>';
      }).join('') : '<div class="empty-hint" style="grid-column:1/-1">No impressions found.</div>';
      ...
      let validation = null, cross = null;
      try {
      ...
      if (validation && validation.errors && validation.errors.length) {
        ...
        valEl.innerHTML =
          '<div class="mono-label" style="margin-bottom:var(--space-3)">' +
            escapeHtml(validation.type) + ' · ' + escapeHtml(validation.status) +
          '</div>' +
          validation.errors.map((e) => {
            ...
            return '<div class="validation-item ' + cls + '">' +
              ...
              '<span>' + escapeHtml(e.msg) + (e.path ? ' <span style="color:var(--text-dim);font-family:var(--font-mono);font-size:10px">[' + escapeHtml(e.path) + ']</span>' : '') + '</span>' +
              '</div>';
          }).join('');
      } else if (validation) {
        $('validationBadge').textContent = '✓';
        valEl.innerHTML = '<div style="text-align:center;padding:var(--space-7);color:var(--success);font-weight:500;">All checks passed — ' + escapeHtml(validation.type) + ' is valid.</div>';
      } else {
        $('validationBadge').textContent = '—';
      }
      ...
      if (Array.isArray(cross) && cross.length) {
        ...
        crossEl.innerHTML = summaryRow + cross.map((c) => {
        ...
        crossEl.innerHTML = '<div class="empty-hint">Crosscheck needs both Bid Request and Bid Response.</div>';
      } else {
        $('crossBadge').textContent = '—';
      }
      ...
      if (!fromHist) {
        ...
        $('hList').innerHTML = historyStore.map((e, i) => {
          const cls = e.status === 'Critical' ? 'critical' : 'healthy';
          return '<div class="history-item" onclick="runAnalysis(historyStore[' + i + '])">' +
            '<div class="history-title">' + escapeHtml(e.title) + '</div>' +
            '<div class="history-meta">' +
              '<span>' + escapeHtml(e.time) + '</span>' +
              '<span class="history-status ' + cls + '">' + escapeHtml(e.status) + '</span>' +
            '</div></div>';
        }).join('');
        if (resVal) toast('Аналіз завершено · ' + (validation ? humanStatus(validation.status) : 'локально'), 'success');
      }
    } catch (e) {
  ...
  function infoRow(k, v) {
    return '<div class="info-row"><span class="info-key">' + escapeHtml(k) + '</span><span class="info-val">' + escapeHtml(v) + '</span></div>';
  }
  ...
  const KADAM = {
    ...
    impFields: [
      ...
      ['ext.subage',  'int',    'req*', 'Push: days since subscription. *Required for push, missing reduces buyout.'],
    ...
    templates: {
      requestNative: {
        id: '8a1f5b9e-2c4d-...', at: 2, cur: ['USD'], tmax: 150,
        imp: [{
          id: '1', secure: 1, bidfloor: 0.005, bidfloorcur: 'USD', tagid: 'zone_42',
          native: {
            request: '{"native":{"ver":"1.1","plcmtcnt":1,"assets":[{"id":1,"required":1,"title":{"len":90}},{"id":2,"required":1,"img":{"type":1,"wmin":192,"hmin":192}},{"id":3,"required":1,"data":{"type":2,"len":140}}]}}',
            ver: '1.1',
          },
          ext: { subage: 2, subage_dt: '2026-04-15', subage_ts: 1744156800 },
        }],
        site: {
          id: 'pub_site_1', domain: 'example.com', page: 'https://example.com/article',
        ...
        device: {
          ...
          language: 'uk', js: 1, devicetype: 2, os: 'macOS', osv: '13.6',
          connectiontype: 2, dnt: 0, lmt: 0,
        },
      ...
      requestPush: {
        id: 'push-req-9c33...', at: 2, cur: ['USD'], tmax: 150,
        imp: [{
          id: '1', secure: 1, bidfloor: 0.001, tagid: 'push_zone_7',
          native: {
            request: '{"native":{"ver":"1.1","plcmtcnt":1,"assets":[{"id":1,"required":1,"title":{"len":50}},{"id":2,"required":1,"img":{"type":1,"wmin":192,"hmin":192}},{"id":3,"required":1,"data":{"type":2,"len":120}}]}}',
            ver: '1.1',
          },
          ext: { subage: 5, subage0: 5, subage_dt: '2026-04-25', subage_ts: 1745020800 },
        }],
        site: { id: 's1', domain: 'pushsite.example', publisher: { id: 'pub_001' }, ext: { idzone: 'push_zone_7' } },
        device: { ua: 'Mozilla/5.0 (Linux; Android 13)', ip: '203.0.113.42', geo: { country: 'POL' }, language: 'pl', devicetype: 4, os: 'Android', osv: '13' },
        user: { id: 'sub_user_abc' },
      },
      responseNative: {
        id: 'request-id-from-bidreq', cur: 'USD',
        seatbid: [{
          seat: '1', group: 0,
          bid: [{
            id: 'bid-1', impid: '1', price: 0.087,
            adid: 'ad_555', crid: 'cr_777', cid: '555555',
            adomain: ['example-advertiser.com'], cat: ['IAB3-1'],
      ...
      feedRequestUrl: '/feed?sid=ENDPOINT_ID&skey=API_KEY&ua=Mozilla%2F5.0&ip=203.0.113.42&uid=user_abc&limit=1&language=uk&pid=pub_001&subage=2&cat=IAB24&page=example.com&format=push',
      feedResponsePush: [{
        id: 'material_777', click_url: 'https://click.kadam.net/c/abc',
        campaign_id: 555555, category: '1368',
      ...
      feedResponseClickunder: { result: { listing: [{ url: 'https://click.kadam.net/cu/xyz', bid: 0.502 }] } },
    },
  };
      ...
      rows.map((row) => '<tr>' +
        row.map((cell, i) => {
          ...
          if (typeof cell === 'string' && /^(req\*?|opt|rec)$/.test(cell)) {
            ...
            return '<td><span class="ref-pill ' + cls + '">' + escapeHtml(cell) + '</span></td>';
          }
  ...
  async function refreshPartners() {
    try {
      ...
      sel.innerHTML =
        ...
        _partnerCache.map((p) => '<option value="' + p.id + '">' + escapeHtml(p.name) + '</option>').join('');
      if ([...sel.options].some((o) => o.value === cur)) sel.value = cur;
    } catch (e) { toast('Не вдалося завантажити список партнерів: ' + e.message, 'error'); }
  }
  ...
  async function refreshSamples() {
    ...
    try {
      ...
      const el = $('savedList');
      if (!list.length) { el.innerHTML = '<div class="saved-empty">Збережених запитів ще немає</div>'; return; }
      ...
      el.innerHTML = list.map((s) => {
        ...
        return (
          '<div class="saved-item" onclick="loadSample(' + s.id + ')">' +
            '<div class="saved-item-actions" onclick="event.stopPropagation()">' +
              '<button class="saved-act-btn" onclick="editSample(' + s.id + ')" title="Перейменувати / змінити партнера">edit</button>' +
              '<button class="saved-act-btn danger" onclick="deleteSample(' + s.id + ')" title="Видалити">×</button>' +
            '</div>' +
            '<div class="saved-item-title">' + escapeHtml(s.title) + '</div>' +
            '<div class="saved-item-meta">' +
              '<span>' + escapeHtml(partnerName(s.partner_id)) + '</span>' +
              (pieces.length ? '<span>·</span><span>' + pieces.join(' · ') + '</span>' : '') +
            '</div>' +
          '</div>'
        );
      }).join('');
    } catch (e) { toast('Не вдалося завантажити запити: ' + e.message, 'error'); }
  }
  ...
  window.openSaveModal = function () {
    ...
    if (!reqVal.trim() && !resVal.trim()) { toast('Нічого зберігати — обидва поля порожні', 'error'); return; }
    const guess = (() => {
      try { const j = JSON.parse(reqVal); return j.id || j.site?.domain || j.app?.bundle || 'sample'; } catch { return 'sample'; }
    })();
    const sel = $('partnerFilter');
    ...
    $('modalRoot').innerHTML =
      '<div class="modal-backdrop" onclick="if(event.target===this)closeModal()">' +
        '<div class="modal-card">' +
          ...
          '<div class="modal-row"><label>назва</label><input id="mTitle" type="text" value="' + escapeHtml(String(guess)) + '"></div>' +
          '<div class="modal-row"><label>партнер</label><select id="mPartner">' + partnerOptionsHtml(presetPartner) + '</select></div>' +
  ...
  window.confirmSave = async function () {
    ...
    try {
      const r = await fetch('api/analyze', {
        ...
        body: JSON.stringify({ bidReq: bid_req ? JSON.parse(bid_req) : {}, bidRes: bid_res ? JSON.parse(bid_res) : {} }),
      });
      const j = await r.json();
    ...
    try {
      await api('POST', 'api/samples', {
        ...
        title, bid_req, bid_res, status, notes,
      });
      closeModal();
      toast('Збережено · ' + title, 'success');
      refreshSamples();
    } catch (e) { toast('Не вдалося зберегти: ' + e.message, 'error'); }
  };
  ...
  window.loadSample = async function (id) {
    try {
      ...
      updateCharCount('bidReq'); updateCharCount('bidRes');
  ...
  window.deleteSample = async function (id) {
    ...
    try { await api('DELETE', 'api/samples/' + id); refreshSamples(); }
    catch (e) { toast('Не вдалося видалити: ' + e.message, 'error'); }
  };
      ...
      $('modalRoot').innerHTML =
        '<div class="modal-backdrop" onclick="if(event.target===this)closeModal()">' +
          '<div class="modal-card">' +
            ...
            '<div class="modal-row"><label>назва</label><input id="mTitle" type="text" value="' + escapeHtml(s.title) + '"></div>' +
            '<div class="modal-row"><label>партнер</label><select id="mPartner">' + partnerOptionsHtml(s.partner_id) + '</select></div>' +
            '<div class="modal-row"><label>нотатки</label><textarea id="mNotes">' + escapeHtml(s.notes || '') + '</textarea></div>' +
            '<div class="modal-actions">' +
              '<button class="btn btn-ghost btn-sm" onclick="closeModal()">скасувати</button>' +
              '<button class="btn btn-primary btn-sm" onclick="confirmEdit(' + s.id + ')">зберегти</button>' +
            '</div>' +
          '</div>' +
        '</div>';
      setTimeout(() => $('mTitle').focus(), 0);
  ...
  window.confirmEdit = async function (id) {
    ...
    try {
      await api('PATCH', 'api/samples/' + id, {
        title, partner_id: partnerId ? Number(partnerId) : null, notes,
      });
      closeModal();
      refreshSamples();
    } catch (e) { toast('Не вдалося зберегти зміни: ' + e.message, 'error'); }
  };

  function partnerListHtml() {
    if (!_partnerCache.length) return '<div class="saved-empty">Партнерів ще немає</div>';
    return _partnerCache.map((p) =>
      '<div class="saved-item" style="cursor:default">' +
        '<div class="saved-item-actions" style="opacity:1">' +
          '<button class="saved-act-btn danger" onclick="deletePartner(' + p.id + ')" title="Видалити">×</button>' +
        '</div>' +
        '<div class="saved-item-title">' + escapeHtml(p.name) + '</div>' +
        '<div class="saved-item-meta"><span>slug · ' + escapeHtml(p.slug) + '</span></div>' +
      '</div>'
    ).join('');
  }
    ...
    $('modalRoot').innerHTML =
      '<div class="modal-backdrop" onclick="if(event.target===this)closeModal()">' +
        '<div class="modal-card">' +
          ...
          '<div id="pList" style="display:flex;flex-direction:column;gap:6px;margin-bottom:var(--space-3);max-height:240px;overflow-y:auto">' + partnerListHtml() + '</div>' +
  ...
  window.confirmAddPartner = async function () {
    ...
    try {
      ...
      $('pName').value = '';
      refreshSamples();
    } catch (e) { toast('Не вдалося додати партнера: ' + e.message, 'error'); }
  };

  window.deletePartner = async function (id) {
    if (!confirm('Видалити цього партнера? Запити що були з ним повʼязані стануть "без партнера" (не видаляються).')) return;
    try {
      await api('DELETE', 'api/partners/' + id);
      await refreshPartners();
      $('pList').innerHTML = partnerListHtml();
      refreshSamples();
    } catch (e) { toast('Не вдалося видалити партнера: ' + e.message, 'error'); }
  };
