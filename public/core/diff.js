/**
 * index.js — deterministic OpenRTB semantic diff.
 *
 * Isomorphic: this one canonical source runs unchanged in Node
 * (`module.exports`) and in the browser (`window.OrtbtoolsDiff`), and
 * scripts/gen-browser-core.js mirrors it verbatim into public/core/ behind a CI
 * parity guard. The Diff tab must apply exactly the rules the tests pin, so
...
(function (root, factory) {
  ...
  else root.OrtbtoolsDiff = factory(root.OrtbtoolsDiffRegistry);
})(globalThis, function (registry) {
  'use strict';

  const { OPENRTB_ARRAY_IDENTITIES, OPENRTB_SET_PATHS } = registry;

  const IDENTITY_BY_PATH = new Map(
    OPENRTB_ARRAY_IDENTITIES.map((entry) => [entry.path, entry.key]),
  );
