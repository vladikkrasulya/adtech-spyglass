# ortbtools — як ним користуватись

**ortbtools** — приватний інспектор OpenRTB-трафіку. Вставив JSON, отримав
...
- **Vendor overlays** — ortbtools підтримує опціональні overlay-правила
...
- Якщо в `bid.adm` є HTML-креатив — ortbtools рендерить його у sandboxed
...
- **Мова інтерфейсу** — перемикач `EN / UK / RU` у нижньому правому куті. Усі три
  локалі — один контракт: інтерфейс, знахідки валідації, crosscheck-повідомлення і
  підказки розмітника перекладені повністю; нестача ключа падає на English, потім
  українську ([feature 015](../specs/015-trilingual-output-parity/spec.md),
...
- **Категорії** — окрема вкладка `categories`. Decoded IAB Content Taxonomy
  лейбли з `cat[] / bcat[] / pcat[]` локалізовані для `en / uk / ru` (наприклад
  `IAB9-11 → Хобі та інтереси → Комікси` для `uk`); якщо переклад конкретного коду
  відсутній, використовується English.
...
| Не підтримує довільні UI-локалі                             | Доступні українська, English і російська; невідомий locale спершу fallback-иться на English, потім на українську                                         |
...
## Які дані ortbtools використовує для аналізу
...
**Твій payload — нічого.** Тіла request і response валідуються в памʼяті й
відкидаються; `/api/analyze` ніколи не записує їх у базу чи лог. Ми зберігаємо
знеособлену аналітику (визначений формат, версію oRTB, кількість знахідок) та
операційний лог запитів, що фіксує метадані запиту — час, метод, шлях, статус і
твою IP-адресу (кожну помилку плюс вибірку успішних викликів) — але ніколи не
payload. Сам Cloudflare Tunnel перед нами теж веде стандартний access-log.
...
ortbtools — це не комерційний продукт. Це open-tools, що допомагають
