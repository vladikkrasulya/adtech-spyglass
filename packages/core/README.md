...
- `strictness` — `'pedantic'` (default, all findings) | `'normal'` (errors + warnings only) | `'lax'` (errors only). Applies to both validator and crosscheck scales (`crit`≡`error`, `warn`≡`warning`, `ok`≡`info`).
