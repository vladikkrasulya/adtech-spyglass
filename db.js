...
const sampleCols =
  'id, partner_id, title, status, notes, created_at, length(bid_req) AS req_len, length(bid_res) AS res_len';
...
const Partners = {
  ...
  create({ name, slug, notes }) {
    ...
    const info = db
      .prepare('INSERT INTO partners(name, slug, notes) VALUES (?, ?, ?)')
...
const Samples = {
  /** @param {{ partnerId?: number | 'unassigned' }} [opts] */
  list({ partnerId } = {}) {
    if (partnerId === 'unassigned') {
      return db
        .prepare(
          `SELECT ${sampleCols} FROM samples WHERE partner_id IS NULL ORDER BY created_at DESC`,
        )
        .all();
    }
    if (partnerId != null) {
      return db
        .prepare(`SELECT ${sampleCols} FROM samples WHERE partner_id = ? ORDER BY created_at DESC`)
        .all(partnerId);
    }
