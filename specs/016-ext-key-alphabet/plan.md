...
| **IV. Deterministic, compatible**        | **ADR required** | Nine new storable labels, the `ambiguous` variant and the saved-mapping variant are public contract changes. FR-028 scopes the contract to four surfaces and requires ADR-015 before code; FR-029–FR-034 pin withdrawal, CLI, export, no-migration, the frozen prompt and the CL-001 boundary; FR-021/SC-010 stay the compatibility floor. Finding IDs, dedup semantics and storage schema are untouched. Core stays network-free. |
