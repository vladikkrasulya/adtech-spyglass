...
- **DEF-200, DEF-201, DEF-202, DEF-203, DEF-205, DEF-260 (resolved 2026-09-08 by [027](../027-inspector-ui-repair/spec.md), no Core change)**: a structured analysis failure now invalidates the previous verdict and stored analysis; an explicit paste starts new lexical provenance; a labelled, keyboard-operable selector reaches every returned seat/bid pair and feed material; documented vendor wrappers and the clickunder creative resolve to a visible card or inert destination, and a PPCmate pop is no longer dressed as a notification; the creative frame carries a localized accessible name. **DEF-245 is not closed** — the redirect script is detected but its identity caption does not reach the measured on-screen text, and the record stays open with an exact signature.
- **DEF-115 (resolved 2026-09-08 by [024](../024-analyze-input-robustness/spec.md), Core 0.43.0)**: `packages/core/categories.js` now guards every category-walk level with `Array.isArray` (`imp`, `seatbid`, and each seat's `bid`), so a syntactically valid payload carrying a non-array `bid`/`seatbid` no longer throws a `forEach` `TypeError` inside the HTTP handler's category decode. `POST /api/analyze` keeps its 200 success envelope and structured findings for `shape-bid-object` and `shape-seatbid-object`; both cases pass at HTTP (and, transitively, in the browser). The ledger records are removed.
- **DEF-204 (resolved 2026-09-08 by [024](../024-analyze-input-robustness/spec.md), Core 0.43.0)**: the Inspector's winning-bid selection in `public/ortbtools.app.js` now resolves `seatbid`/`bid` with `Array.isArray` guards and defaults a null/undefined/non-object first bid to `{}`, so a malformed first bid (`shape-bid-null`, `shape-bid-object`, an empty first `bid[]` in `multiplicity-empty-bid-array`) reaches structured validation instead of throwing `Cannot read properties of … (reading 'cur')` before the Analyze POST. All three cases pass at the browser layer. The ledger records are removed.
- **DEF-300 (resolved 2026-09-08 by [024](../024-analyze-input-robustness/spec.md), Core 0.43.0)**: `modules/analyze/handler.js` now treats a present scalar `bidRes` (e.g. a bare `42`) as a submitted response, so `validate()` rejects it with `payload.invalid_root` and `crosscheck` reports `crosscheck.no_response`, matching Core's own direct behaviour. `mut-input-shape-number-root-response` passes at HTTP. The HTTP corpus harness was extended to represent a scalar response side (a merged envelope cannot express a per-side `invalid` root status, which stays asserted at the Core layer). The ledger record is removed.
- **DEF-303 (resolved 2026-09-08 by [024](../024-analyze-input-robustness/spec.md), Core 0.43.0)**: `lib/http.js#readJson` no longer destroys the socket when the 2 MiB cap is exceeded; it rejects with `payload_too_large` and drains without buffering, so the handler delivers the documented `400 payload_too_large` error envelope instead of an `ECONNRESET`. `encoding-request-body-oversized` passes at HTTP. The ledger record is removed.
...
- **DEF-160, DEF-161, DEF-181, DEF-460 (resolved 2026-09-08 by [025](../025-format-detect-vendor-dialects/spec.md), Core 0.44.0)**: the format-detection and feed-dispatch paths are aligned. `detectFeedFormat` now honours the `image_url`/`icon_url` aliases the push validator already accepts (DEF-160); the `clickurl` alias recognized by `format-detect.js` is now also recognized by `detect.js`, `rules-feed.js` and the Inspector's material finder (DEF-181); a bare materials array dispatches each element by its shape, so a RichAds `bid_price`/`notification_url` element is validated as a bid-price shape instead of being force-fit through push-material field names (DEF-161); and a standalone Native 1.x `adm` body is tagged `native` even without the optional `mtype` (DEF-460). Their six cases pass normatively at their applicable layers — the two-card `inpage-x-card-feed-array` retains only the separate browser selection gap DEF-201, which the DEF-181 record had masked — and the four ledger records are removed; no finding id, level or message changed. The same alias alignment also corrects the response-format detection of the still-open DEF-107 Kadam cases, whose records are re-pinned to their residual request-decoder gap.
...
- **DEF-113, DEF-194, DEF-111, DEF-192, DEF-130 (resolved 2026-09-08 by [026 wave A](../026-validation-crosscheck/verification.md), Core 0.45.0)**: actual returned VAST MIME/duration/protocol facts are compared with the selected video offer; banner/audio content contradictions and explicit buyer-seat restrictions receive findings; mixed offers apply the selected media constraints; valid NonLinear-only InLine VAST no longer requires Linear MediaFiles. All eleven cases pass their unchanged semantic and applicable browser expectations. Three unbound fixture ID symbols are bound to production IDs; payloads, levels, paths and parameter assertions remain intact. The five ledger records are removed. Wave B and independent peer-owned deviations remain open.

- **DEF-195, DEF-150, DEF-190, DEF-198, DEF-109, DEF-170, DEF-301, DEF-197 (resolved 2026-09-08 by 026 wave B, Core 0.45.0; final layer evidence in [026 verification](../026-validation-crosscheck/verification.md))**: markup type domains and offered-media contradictions are diagnosed; structured 2.x Native participates in presence and required-asset crosscheck; repeated buyer seats, whitespace markup, unassigned no-bid codes and malformed COPPA receive findings; the explicit EXADS popunder form and valid pop notice-only response retain their documented behavior. All nine cases pass their unchanged Core, HTTP and applicable browser requirements; their eight records are removed. The remaining DEF-151 record is described below.

- **DEF-151 (partial 026 repair, still open)**: structured AdCOM Native no longer requires extra display markup and its required assets receive the existing Native crosscheck. The five cases retain only their original recognition and actual preview deviations in peer-owned files. Their normative expectations and remaining signatures are unchanged; the ledger record remains active.

- **DEF-245 (resolved 2026-09-08, App 1.19.4)**: `setDims()` was sizing the whole `.preview-safe` box from the bid's own (often near-zero, e.g. 1×1) creative dimensions even on the popRedirect branch, so `.preview-text`/`.preview-text-body` — the inert note carrying the redirect destination, appended beside the already-correctly-zero-footprint iframe — inherited a collapsed ancestor box and measured a `visible()` width of 0 even though its text content was already correct. `public/ortbtools.app.js` now branches `setDims(300, 250)` (the same fallback already used by the VAST/JSON/URL/unidentified inert-text branches) when `popRedirect` is truthy, before appending the note, leaving the iframe's own inline zero-footprint style and the sandbox refusal untouched. All four cases (`pop-x-openrtb-adtype-clickunder-location`, `pop-x-openrtb-adtype-popunder-window-open`, `pop-x-openrtb-flag-popunder-bool`, `pop-x-openrtb-shape-flag-allowshock`) pass normatively at the browser layer, including their `sandboxRefusals` assertion; the ledger record is removed.

- **DEF-180 (preview half resolved 2026-09-08 by 029; format-tagging half still open)**: `public/ortbtools.app.js` now resolves the In-Page Push carrier — an OpenRTB bid with no `adm` and no Native object whose creative travels in `bid.ext` — using the claim predicate and the six-role alias table of the product’s own `packages/core/dialects/inpage-push.js`, and renders it through the existing synthetic push card. Roles are normalised before rendering rather than by widening `isPushMaterialShape`, because the two alias tables disagree on `text` (a title in the dialect, a description in the flat-feed renderer) and because the flat gate is what keeps vendor banner and pop wrappers from being dressed as cards (DEF-203); a corpus-wide measurement confirms the new predicate reaches exactly the five documented carrier bids and no other bid in the 257-case corpus. All five cases (`inpage-x-widget-openrtb-canonical`, `-openrtb-aliases`, `-iab-contract`, `-inapp`, `cover-vendor-inpage-openrtb26`) now measure `kind=push`, `rendered=partial` and a visible headline marker; their preview and identity signatures are retired. What remains on the record is Core detection only: `format-detect.js` rule 5 reads the widget/zone placement signal for the JSON-feed shape and is gated off for an `imp[]`/`seatbid[]`-shaped payload.

- **DEF-106, DEF-107, DEF-151, DEF-441 (resolved 2026-09-09 by 030; the ledger is now empty)**: the last four records were one sentence each — the response carries a creative the Inspector could not read. `public/ortbtools.app.js` now unwraps the OpenRTB 3.0 envelope and renders an AdCOM `media.ad.display.native` creative through the existing native renderer (AdCOM’s `asset`/`image` names mapped onto the oRTB Native 1.x shape rather than a second renderer added); renders a documented vendor Native material — headline, picture, landing link and price, with no push click alias — as a native card; renders a documented vendor banner wrapper — price, click and picture, with no notification text — as markup through the sealed frame, where its remote artwork is blocked and explained exactly as a banner’s is; and shows a material’s own title and description above a bare destination, carried beside the creative body so the classifier, the behaviour engine and the static scanner all still read the response’s own bytes. The push qualification gate is untouched, and a corpus-wide measurement kept as a standing test confirms the four new predicates reach exactly the ten documented cases and no other bid or material in the 257-case corpus. Two of DEF-107’s cases closed instead by correcting an expectation the wire cannot support, under [ADR-017](../decisions/ADR-017-format-tags-follow-the-wire.md): the Kadam feed request omits the vendor’s optional format parameter (whose documented values contain no in-page) and the response materials carry no placement signal, so the in-page-versus-push distinction exists only in vendor account provisioning and never on the wire.
