...
## Maintenance follow-through (032; 2026-09-09)

The independent baseline review at `2bd93d6` found **4 fixed · 26 open · 1 superseded · 2 unverifiable** among the 33 original records. Features 021–031 had closed the corpus defect ledger, not this inventory.

**Current state: zero open records in the original 33-record inventory.** Feature [032](../032-close-cleanup-inventory/spec.md) closed the 26 originally open entries with individual [dispositions and evidence](../032-close-cleanup-inventory/inventory.md), shipped in v1.22.0 / `9899191` on 2026-09-09. It also repairs the separately discovered `negative-floor-unflagged`: negative impression/item and matched-deal floors receive `floor.negative` and cannot earn a satisfied-floor verdict. CL-08 receives a capability-boundary decision; CL-09 receives a bounded assertion inventory with an executable reference guard. Neither disposition pretends an unbuilt capability or untested combination was implemented.

The [verification record](../032-close-cleanup-inventory/verification.md) records successful full repository/corpus gates, exact-commit hosted CI, verified pre-deploy backup, deployment and smoke. The four prior fixes (`scattered-format-07`, `fallbacks-silent-01`, `CL-01`, `CL-10`) and superseded `CL-03` are retained and not counted as new work.

The evidence, original locations, priorities and estimates below are **historical intake descriptions**, preserved so each closure can be compared with its original claim. Their present-tense failure descriptions and old “no test” observations are not statements about the current implementation. The current per-ID evidence above supersedes them. Dropped candidates and unrelated product work remain outside this named closure.
...
Ten earlier architecture proposals from the first pass (CL-01 to CL-10) are retained in the Refactoring section; CL-08 and CL-09 were preliminary at intake; feature 032 now supplies concrete affected paths and assertions; they overlap with several verified items and the overlap is noted there.
