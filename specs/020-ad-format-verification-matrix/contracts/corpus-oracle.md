...
- All applicable pairwise cells and standard protocol/context triples require cases; the axes regression gate ignores focused case filters. Out-of-projection creative kinds are not declared impossible input values.
- Positive-fixture source properties are checked independently in tests/corpus-fixture-contract.test.js; product output is not its oracle. Context metadata, AdCOM types/Native nesting and VAST Inline identifiers must agree with the cited source.
- The dedicated audit now has nine phases, including that independent fixture-contract gate. Known defects still require measured per-layer signatures; closing a coverage cell does not change its conformance outcome.
