...
- [ ] T006 Baseline: `p0-tri-v1-31b-cpu-tt` done — 26/30 answered, all correct; **4 of 30 hit the 3600 s timeout** (t02 ×2, t04 ×2: the same two cases 8B-with-thinking mis-labels). 31B with thinking does not terminate on them within an hour. Thinking hurts triage on both tiers; on 31B it costs hours. `p0-ext-v2-31b-cpu-tf` in flight; chain ends with a deep-resident check.
...
- [ ] T017 [FR-006] `taxonomy/ADJUDICATION.md` — human report `old rule → facts → projected → rationale` and migration proposal; canonical corpus untouched.
...
- [ ] T021 [FR-007] Freeze manifests (`frozen: true`, hashes) before any tuning; record in split files.
...
- [ ] T041 [FR-007] Holdout opened once for the final candidate; `holdout_opened` recorded.
- [ ] T042 Final report: before/after, confusion matrices, Pareto quality/latency/RAM, all rejected hypotheses, migration proposal, rollback, exact host state (Disk/RAM/timestamp/node); every unmet gate labelled with error taxonomy and next verifiable step.
