...
setpriv --reuid "$U" --regid "$U" --groups "$U" sh -c \
  "umask 027; : > '$T/ortbtools.db'; : > '$T/ortbtools.db-wal'; : > '$T/ortbtools.db-shm'; : > '$T/deploy-state.env'; chmod 600 '$T/deploy-state.env'"
...
echo "DB_MODE=$(stat -c '%a %u:%g' "$T/ortbtools.db")"
echo "WAL_MODE=$(stat -c '%a %u:%g' "$T/ortbtools.db-wal")"
echo "SHM_MODE=$(stat -c '%a %u:%g' "$T/ortbtools.db-shm")"
echo "GRAFANA_READ_DB=$(r "$GRAFANA" "$GRAFANA" "$G" "$T/ortbtools.db")"
echo "GRAFANA_READ_WAL=$(r "$GRAFANA" "$GRAFANA" "$G" "$T/ortbtools.db-wal")"
echo "GRAFANA_READ_SHM=$(r "$GRAFANA" "$GRAFANA" "$G" "$T/ortbtools.db-shm")"
echo "GRAFANA_LIST_DIR=$(l "$GRAFANA" "$GRAFANA" "$G" "$T")"
echo "GRAFANA_NOGROUP_READ=$(r "$GRAFANA" "$GRAFANA" "$GRAFANA" "$T/ortbtools.db")"
echo "STRANGER_READ=$(r "$STRANGER" "$STRANGER" "$STRANGER" "$T/ortbtools.db")"
echo "GRAFANA_READ_DEPLOYSTATE=$(r "$GRAFANA" "$GRAFANA" "$G" "$T/deploy-state.env")"
echo "APP_WRITE=$(setpriv --reuid "$U" --regid "$U" --groups "$U" sh -c "echo x >> '$T/ortbtools.db'" >/dev/null 2>&1 && echo OK || echo FAIL)"

# restart/checkpoint: delete + recreate WAL/SHM as the app (umask 027) → must
# inherit group via setgid + stay 0640
setpriv --reuid "$U" --regid "$U" --groups "$U" sh -c \
  "rm -f '$T/ortbtools.db-wal' '$T/ortbtools.db-shm'; umask 027; : > '$T/ortbtools.db-wal'; : > '$T/ortbtools.db-shm'"
echo "WAL_RECREATE_MODE=$(stat -c '%a %u:%g' "$T/ortbtools.db-wal")"
echo "GRAFANA_READ_RECREATED_WAL=$(r "$GRAFANA" "$GRAFANA" "$G" "$T/ortbtools.db-wal")"
